...
{\tiny 2-LAP \\ \grey{2-OSP} \\ \grey{2-UDFLP} \\ \grey{2-UMFLP} \\ \grey{2-UKP} \\ \grey{$p$-PATHS} \\}
};
...
{\tiny 2-LAP[prz2008] \\ \grey{2-OSP[was1980]} \\ \grey{2-UDFLP[gan2012]} \\ \grey{2-UMFLP[del2017]} \\ \grey{2-UKP} \\ \grey{$p$-PATHS} \\}
};
...
\item \grey{2-OSP with here $1\mid . \mid (\Sigma C_i,T_{max})$  [was1980]; in Julia:\vspace{1mm}\\
...
\item Launch Julia
\begin{enumerate}
  \item open a console on your computer 
  \item when the prompt is ready, type in the console \\
      \green{\texttt{julia}}
\end{enumerate}
\medskip
...
\item Before the first use of \texttt{vOptSolver}, add both following packages:
\begin{enumerate}
  \item when the prompt \textit{julia} is ready, type in the console \\ 
     \green{\texttt{Pkg.clone("\url{http://github.com/vOptSolver/vOptGeneric.jl}")}}\\
     \green{\texttt{Pkg.clone("{http://github.com/vOptSolver/vOptSpecific.jl}")}}
     \green{\texttt{Pkg.build("{vOptSpecific}")}}         
\end{enumerate}  
...
%                                     \item $z_k, \ k=1,\dots,p$ of integers:  performances 
                                     \item $\sigma_i (\ i=1,\dots,n$): integers;  permutation coding ($x_{ij}=1 \Leftrightarrow \sigma_i=j$)
                                \end{itemize}
                           \end{itemize}               
...
# --- create a MOMILP and set the model ---
  MOMILP = Model(solver=<working on it>) 
  @variable(MOMILP, x1 >= 0)
  @variable(MOMILP, x2 >= 0, Int)
  @setObjective(MOMILP, Min, c11*x1 + c12*x1)
  @addObjective(MOMILP, Min, c21*x1 + c22*x1)  
  @addConstraint(MOMILP, t11*x1 + t12*x2 <= d1)
  @addConstraint(MOMILP, t21*x1 + t22*x2 <= d2)
\end{verbatim}  
}
%%
%%
...
# --- create a MOLP and set the model ---
  MOLP = Model(solver=<working on it>) 
  @variable(MOMILP, x1 >= 0)
  @variable(MOMILP, x2 >= 0)
  @setObjective(MOLP, Min, c11*x1 + c12*x1)
  @addObjective(MOLP, Min, c21*x1 + c22*x1)  
  @addConstraint(MOLP, t11*x1 + t12*x2 <= d1)
  @addConstraint(MOLP, t21*x1 + t22*x2 <= d2)
\end{verbatim}  
}
%%
%%
...
# --- create a MOIP and set the model ---
  MOIP = Model(solver=GLPKSolverMIP()) 
  @variable(MOIP, x1 >= 0, Int)
  @variable(MOIP, x2 >= 0, Int)
  @setObjective(MOIP, Min, c11*x1 + c12*x1)
  @addObjective(MOIP, Min, c21*x1 + c22*x1)  
  @addConstraint(MOIP, t11*x1 + t12*x2 <= d1)
  @addConstraint(MOIP, t21*x1 + t22*x2 <= d2)
\end{verbatim}  
}
...
% ================================================================
% 
\begin{frame}[fragile=singleslide]{MOMILP-MOLP-MOIP $\mid$ Data $\mid$ Example (text file; format MOP)}
...
\begin{itemize}
\item  \red{load2MOP}\\
          Load an instance of a 2-MOP from the file \texttt{fname}    \\
           \texttt{jumpModel  = \texttt{\red{load2MOP}( fname ) }} 
           \medskip
\end{itemize}
...
# --- solve the model ---
  status = solve(MOMILP)
# --- Get the results ---
  println("z1  = ", getObjectiveValue(MOMILP))
...
# --- solve the model ---
  status = solve(MOLP)
# --- Get the results ---
  println("z1  = ", getObjectiveValue(MOLP))
  println("x1 = ", getvalue(x1))
  println("x2 = ", getvalue(x2))
\end{verbatim}
}
...
# --- solve the model ---
  status = solve(MOIP)
# --- Get the results ---
  println("z1  = ", getObjectiveValue(MOIP))
  println("x1 = ", getvalue(x1))
  println("x2 = ", getvalue(x2))
\end{verbatim}
}
...
\begin{itemize}
  \item 2-MILP: a mixed nondominated set (composed of edges that are either closed, half-open, open or reduced to a point)
  \item 2-LP: a continuous nondominated set (composed of edges)
...
\begin{itemize}
  \item v1.0: July 2016; first prototype; not opened to the public
    \medskip
  \item v2.0: July 23, 2017; first stable version;  released to the public with
    \begin{itemize}
       \item a depot on github, webpages, and an user documentation    
       \item JuMP extended to multiple objectives (vOptGeneric)
       \item primitive for loading a non-structured problem in std MOP format
...
% ================================================================
% 
\begin{frame}{Links and contact}

\begin{itemize}
\item Homepage of the ANR/DFG research project ``vOpt'': \\
         \url{http://vopt-anr-dfg.univ-nantes.fr}
         \bigskip
\item Repository of vOptSolver:\\
         \url{http://github.com/vOptSolver}
\item Homepage of vOptSpecific:\\
         \url{http://voptsolver.github.io/vOptSpecific/}
\item Homepage of vOptGeneric:\\
         \url{http://voptsolver.github.io/vOptGeneric/}              
...
\item Contact concerning vOptSolver:\\

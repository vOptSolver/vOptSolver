...
# --- create a MOMILP and set the model ---
  ...
  @objective(MOMILP, Min, c11*x1 + c12*x1)
...
# --- create a MOLP and set the model ---
  ...
  @objective(MOLP, Min, c11*x1 + c12*x1)
...
# --- create a MOIP and set the model ---
  ...
  @objective(MOIP, Min, c11*x1 + c12*x1)
...
\blue{parseMOP}\\
          Load an instance (MOP format) from the file \texttt{fname}    \\
           \texttt{jumpModel  = \texttt{\blue{parseMOP}( fname , solver=<solver to invoke> ) }} 
           \medskip
%\end{itemize}
